...
export async function botPickPlayer({ 
    team_id
  }: { 
    team_id: number,
}) {
    console.log("BOT IS ABOUT TO PICK A PLAYER")
  try {
    const token = localStorage.getItem('token');
    ...
    res = await axios.post(`http://3.20.227.225:8082/api/bots/${team_id}/pick-player`, {},
    {
      headers: {
          Authorization: `Bearer ${token}`,
      },
    ...
    console.log("BOT'S PLAYER PICK DATA: ", res?.data);
