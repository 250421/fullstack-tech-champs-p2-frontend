...
const fetchAuthStatus = async (): Promise<{ isAuthenticated: boolean; user: any | null; userId: number | null;username: string | null }> => {
  const token = localStorage.getItem("token");

  // If there's no token, return unauthenticated state
  if (!token) {
    return { isAuthenticated: false, user: null, userId: null,username:null };
  }
  ...
  try {
    ...
    return {
      ...
      user: res.data,
      userId,username,
       // Pass the userId extracted from the token
    };
  } catch (error) {
